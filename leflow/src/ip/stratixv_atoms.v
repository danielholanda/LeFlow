...
// Deactivate the following LEDA rules for stratixv_lcell_comb.v
...
// Deactivate the following LEDA rules for stratixv_ram_block.v
// G_521_3B: Use uppercase letters for all parameter names
// leda G_521_3_B off
...
generic_28nm_hp_mlab_cell_impl my_lutram0
(
	.portadatain(portadatain),
	.portaaddr(portaaddr),
	.portabyteenamasks(portabyteenamasks),
...
// Deactivate the following LEDA rules for stratixv_io_atom.v
// G_521_3B: Use uppercase letters for all parameter names
...
assign delay_x2_in		= (half_rate_mode == "true")? dfflo_tmp : dffhi_tmp;	   
assign delay_x0_in		= (half_rate_mode == "true")? dffhi_tmp : dfflo_tmp;	   
assign sel_mux_lo_in	= (half_rate_mode == "true")? delay_x2_out : delay_x0_out; 
assign sel_mux_hi_in	= (half_rate_mode == "true")? delay_x0_out : delay_x2_out; 
...
module    stratixv_dqs_config    (
	...
	parameter dca_calibration_block_input_select = "0";
    parameter    use_pvt_compensation = "false";
	...
	input        update;
	input [20:0] dftin;
	input [6:0]  delayctrlin;  // SPR 384469
	input 		 rankselectwrite;
	input [7:0]  coremultirankdelayctrlin;
	input        corerankselectreadin;
	input        rankclkin;
	input        rankselectread;
	input 		 calibrationdone;
	...
	output [3:0] dutycycledelaysetting; // SPR:381998
	...
	output [5:0] octdelaysetting1; // SPR 362735
	output [5:0] octdelaysetting2; // SPR 362735
	...
	output [1:0] postamblezerophasesetting; // SPR 378197
	...
	output [6:0] dftout;
	output [7:0] coremultirankdelayctrlout;
	output       rankselectreadout;
    ...
    defparam inst.lpm_type = lpm_type;
    defparam inst.dca_calibration_block_input_select = dca_calibration_block_input_select;
    defparam inst.use_pvt_compensation = use_pvt_compensation;
...
module    stratixv_dqs_enable_ctrl    (
    ...
    input   [7:0] coredqsenabledelayctrlin;
    input   [7:0] coredqsdisablendelayctrlin;
    output   [7:0] coredqsenabledelayctrlout;
    output   [7:0] coredqsdisablendelayctrlout;
    output   rankclkout;
...
//input and output port declaration
	...
	stratixv_pll_reconfig_encrypted 
	#(
		.fractional_pll_index(fractional_pll_index)
	) 
	stratixv_pll_reconfig_encrypted_inst (
		.addr(addr),
        .atpgmode ( atpgmode ),
		.byteen(byteen),
		.clk(clk),
...
module    stratixv_io_config    (
    ...
    parameter    use_read_vt_tracking = "false";
    parameter    use_pvt_compensation = "false";
    ...
    stratixv_io_config_encrypted inst (
        ...
        .dataout(dataout) );
    defparam inst.lpm_type = lpm_type;
    defparam inst.use_read_vt_tracking = use_read_vt_tracking;
    defparam inst.use_pvt_compensation = use_pvt_compensation;
...
module    stratixv_lvds_rx    (
    ...
    defparam inst.dpa_config = dpa_config;
...
module    stratixv_tsdblock    (
    ...
    defparam inst.poi_cal_temperature = 85;
    defparam inst.tsdblock_mode = "corner_sense";
    defparam inst.use_dft_compout = "false";
    defparam inst.lpm_type = lpm_type;
...
module    stratixv_oscillator    (
    ...
    stratixv_oscillator_encrypted inst (
        .oscena(oscena),
        .clkout(clkout),
        .clkout1(clkout1));
        
endmodule //stratixv_oscillator
